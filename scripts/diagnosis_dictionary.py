				...
				while index < 10:
					if visit[all_diag[index]] == '':
						...
						if diag not in summary:
							...
							summary[diag]["total_cost"] = float(visit[claimIndex])
...
def createJson(dictionary):
	...
	#print dictionary['total_uniques'],  dictionary['total_claims'] 
	
	with open('allpatients.json', 'wb') as fp:
...
if __name__=='__main__':
	...
	dictionary = parsePatientFiles(x, chronic, summary) # either give method one merged list of inpatient and outpatient files, inpatient or just outpatient 
